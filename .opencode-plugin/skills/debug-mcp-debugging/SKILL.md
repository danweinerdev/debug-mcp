...
**Precondition (macOS/Linux):** `lldb-dap` must be discoverable (on `PATH`, or via
`LLDB_DAP_PATH`). The server detects it lazily at `launch`/`attach`; if detection
fails the tool returns an error naming every path it searched. On Windows the
OS-bundled DbgEng is used — nothing to install; WinDbg specifics (crash dumps,
kernel targets, modules, backend selection) are in **debug-mcp-windbg**.
