...
|---|---|---|
| The call stack | `backtrace` | `thread_id`, `levels` |
| List threads | `threads` | — |
| Variables in scope | `variables` | `frame_index`, `scope`, `depth`, `filter` |
| Evaluate an expression | `evaluate` | `expression` (required), `frame_index` |
| What the program printed | `read_output` | — |
